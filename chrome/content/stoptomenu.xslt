...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<xsl:template match="/">
		<listbox xmlns="http://www.mozilla.org/keymaster/gatekeeper/there.is.only.xul" seltype="multiple">
			<xsl:for-each select="bustime-response/stop">
				<listitem type="checkbox">
					<xsl:attribute name="label">
						<xsl:value-of select="stpnm" />
					</xsl:attribute>
					<xsl:attribute name="value">
						<xsl:value-of select="stpid" />
					</xsl:attribute>
				</listitem>
			</xsl:for-each>
		</listbox>
